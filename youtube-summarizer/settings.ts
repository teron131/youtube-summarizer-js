...
const DEFAULTS = {
  ...
  scrapecreatorsTranscriptUrl: "https://api.scrapecreators.com/v1/youtube/video/transcript",
...
function asReasoningEffort(value: string | undefined): OpenRouterReasoningEffort {
...
export function getSettings(): AppSettings {
  ...
  cachedSettings = {
    ...
    llmTimeoutSeconds: asNumber(readEnv("LLM_TIMEOUT_SECONDS"), DEFAULTS.llmTimeoutSeconds),
    taskTimeoutSeconds: asNumber(readEnv("TASK_TIMEOUT_SECONDS"), DEFAULTS.taskTimeoutSeconds),
    scrapecreatorsTranscriptUrl:
      readEnv("SCRAPECREATORS_TRANSCRIPT_URL") ?? DEFAULTS.scrapecreatorsTranscriptUrl,
    supadataTranscriptUrl: readEnv("SUPADATA_TRANSCRIPT_URL") ?? DEFAULTS.supadataTranscriptUrl,
    openrouterSummaryModel: readEnv("OPENROUTER_SUMMARY_MODEL") ?? DEFAULTS.openrouterSummaryModel,
    openrouterReasoningEffort: asReasoningEffort(readEnv("OPENROUTER_REASONING_EFFORT")),
    geminiSummaryModel: readEnv("GEMINI_SUMMARY_MODEL") ?? DEFAULTS.geminiSummaryModel,
    geminiThinkingLevel: readEnv("GEMINI_THINKING_LEVEL") ?? DEFAULTS.geminiThinkingLevel,
...
export function hasScrapeCreators(settings: AppSettings = getSettings()): boolean {
...
export function hasAnyTranscriptProvider(settings: AppSettings = getSettings()): boolean {
  return hasScrapeCreators(settings) || hasSupadata(settings);
}

export function llmTimeoutMilliseconds(settings: AppSettings = getSettings()): number {
  return settings.llmTimeoutSeconds * 1000;
}

export function mcpGoogleScopes(settings: AppSettings = getSettings()): string[] {
