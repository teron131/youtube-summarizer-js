...
function hasSegmentTranscript(segments: TranscriptSegment[]): boolean {
  return segments.some((segment) => Boolean(segment.text && segment.text.trim()));
}
...
function hasTranscript(result: YouTubeScrapperResult | null): boolean {
  ...
  return Boolean(result.transcript_only_text && result.transcript_only_text.trim());
}
...
export async function scrapeYoutube(youtubeUrl: string): Promise<YouTubeScrapperResult> {
...
export async function extractTranscriptText(youtubeUrl: string): Promise<string> {
