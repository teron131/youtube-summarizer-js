...
import type { TranscriptSegment, YouTubeScrapperResult } from "./scrapeCreators.js";

export async function fetch_supadata(video_url: string): Promise<YouTubeScrapperResult | null> {
  ...
  try {
    const response = await fetch(`${settings.supadataTranscriptUrl}?${query.toString()}`, {
      method: "GET",
      headers: {
    ...
    return {
      ...
      availableLangs: Array.isArray(data.availableLangs)
        ? data.availableLangs.filter((lang): lang is string => typeof lang === "string")
        : null,
      success: true,
      type: "video",
    };
