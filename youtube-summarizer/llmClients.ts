...
export function OpenRouterEmbeddingsModel(model = "openai/text-embedding-3-small"): {
  apiKey: string | null;
  baseURL: string;
  model: string;
} {
  if (!isOpenRouter(model)) {
    throw new Error(`Invalid OpenRouter model format: ${model}. Expected PROVIDER/MODEL`);
  }
