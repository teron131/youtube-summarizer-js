function buildContextBlock(title?: string | null, description?: string | null): string {
  const parts: string[] = [];
  if (title) {
...
export function getGeminiSummaryPrompt(
  ...
  const normalizedLanguage =
    targetLanguage === "auto" || targetLanguage === "en" || targetLanguage === "zh"
      ? targetLanguage
      : "auto";
...
export function getLangchainSummaryPrompt(
  ...
  if (targetLanguage) {
    const languageInstruction =
      ...
      )[targetLanguage] ?? "Use the same language as the transcript, or English if unclear";
