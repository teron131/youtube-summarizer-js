...
import { parseSummary, parseSummaryJson, type Summary } from "./schemas.js";
import { getSettings, llmTimeoutMilliseconds } from "./settings.js";

const USD_PER_M_TOKENS_BY_MODEL: Record<string, { input: number; output: number }> = {
  "gemini-3-flash-preview": { input: 0.5, output: 3 },
  "gemini-3-pro-preview": { input: 2, output: 12 },
};

function calculateCost(model: string, promptTokens: number, totalTokens: number): number {
...
function extractUsageMetadata(
  model: string,
  usageMetadata: { promptTokenCount?: number; totalTokenCount?: number } | undefined,
...
export async function summarizerGemini(
  ...
  try {
    ...
    return {
      summary,
      metadata: extractUsageMetadata(settings.geminiSummaryModel, response.usageMetadata),
    };
  } catch {
    return { summary: null, metadata: null };
  }
