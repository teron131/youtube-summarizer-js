...
import { cleanYoutubeUrl, isYoutubeUrl, processingTime } from "../youtube-summarizer/utils.js";
...
export const TOOL_DESCRIPTIONS = {
  ...
  summarize: "Generate summary from a YouTube URL using internal provider fallback.",
} as const;
...
export async function scrapeTool(url: string): Promise<ToolResult> {
  ...
  if (!hasTranscriptProviderKey()) {
    throw new Error("Config missing: SCRAPECREATORS_API_KEY or SUPADATA_API_KEY");
  }
...
export async function summarizeTool(url: string): Promise<ToolResult> {
  ...
  if (provider === "gemini") {
    const { summary, metadata } = await summarizerGemini(normalizedUrl, resolvedTargetLanguage);
    if (!summary) {
  ...
  const transcript = await extractTranscriptText(normalizedUrl);
  const summary = await summarizerOpenRouter(transcript, resolvedTargetLanguage);
  return buildSummarySuccessResult(provider, summary, resolvedTargetLanguage, buildMetadata(start));
}
