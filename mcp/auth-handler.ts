...
const DEFAULT_GOOGLE_AUTHORIZATION_URL = "https://accounts.google.com/o/oauth2/v2/auth";
...
function oauthConfigHealth(request: Request, env: WorkerOAuthEnv): Record<string, unknown> {
...
async function exchangeGoogleToken(
  ...
  const clientSecret = ensureSecret(env.GOOGLE_CLIENT_SECRET, "GOOGLE_CLIENT_SECRET");
...
export const googleOAuthDefaultHandler = {
  async fetch(request: Request, envInput: unknown, _ctx: any): Promise<Response> {
    void _ctx;
    ...
    switch (pathname) {
      ...
      case "/callback": {
        ...
        if (!code || !state) {
          return new Response("Missing callback parameters", {
            status: 400,
          });
        }
        ...
        if (!serializedRequest) {
          return new Response("Authorization session expired", {
            status: 400,
          });
        }
